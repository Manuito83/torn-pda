...
class PrefsDatabase {
  ...
  static final Lock _dbInitLock = Lock();
  static final Lock _opLock = Lock();

  /// Ensure the database is initialized
  static Future<Database> get database async {
    if (_database != null && _isInitialized) return _database!;

    return await _dbInitLock.synchronized(() async {
      if (_database != null && _isInitialized) return _database!;

      try {
        final Directory appDocDir = await getApplicationDocumentsDirectory();
        final Directory dbDir = Directory('${appDocDir.path}/database');
        if (!await dbDir.exists()) {
          await dbDir.create(recursive: true);
        }
        final String dbPath = '${dbDir.path}/torn_pda_preferences.db';
  ...
  static Future<String> getString(String key, String defaultValue) async {
    return await _opLock.synchronized(() async {
      try {
