            ...
            for (let id in factionsList) {
                ...
                try {
                    ...
                    if (factionsList[id].timestamp === undefined) {
                        promises.push(db.ref(`retals/factions/${ownFactionId}/api`).set(""));
                    }

                    if (factionsList[id].api === undefined) {
                        promises.push(db.ref(`retals/factions/${ownFactionId}/timestamp`).set(0));
                    }
                    ...
                    if (apiKey === "") {
                        ...
                        if (subscribers.length === 0) {
                            promises.push(refFactions.child(ownFactionId).remove());
                            continue;
                        }
                        ...
                        if (numberOfHosts === 0) {
                            ...
                            promises.push(db.ref(`retals/factions/${ownFactionId}/noHostWarning`).set(true));
                            continue;
                        } else if (numberOfHosts > 0) {
                            // Rearm the no-hosts warning
                            promises.push(db.ref(`retals/factions/${ownFactionId}/noHostWarning`).set(false));
                        }
                        ...
                        promises.push(db.ref(`retals/factions/${ownFactionId}/api`).set(lastActiveUserApiKey));
                    }

                    // Update the faction timestamp
                    promises.push(db.ref(`retals/factions/${ownFactionId}/timestamp`).set(currentDateInMillis));
